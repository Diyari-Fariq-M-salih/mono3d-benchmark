...
mkdir -p "$log_dir"
run_log="$log_dir/${timestamp}_qcar.log"
...
prepare_cmd=("python3" "$repo_root/scripts/prepare_qcar_evry_svo.py" "--write-configs")
for seq in "${sequences[@]}"; do
  prepare_cmd+=("--sequence" "$seq")
done
prepare_json="$log_dir/${timestamp}_qcar_prepare.json"
"${prepare_cmd[@]}" | tee "$prepare_json"

roscore_started=0
if ! timeout 2 rosparam list >/dev/null 2>&1; then
  roscore >"$log_dir/${timestamp}_qcar_roscore.log" 2>&1 &
  roscore_pid=$!
  roscore_started=1
...

{
  ...
  echo "Experiment: mono3d_qcar_evry_mono"
  python3 "$svo_benchmark_script" mono3d_qcar_evry_mono.yaml || true
  emit_sanity_reports "mono3d_qcar_evry_mono" || true
} 2>&1 | tee "$run_log"
